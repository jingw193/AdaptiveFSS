...
class DatasetCOCO_finetune(Dataset):
    ...
    def build_class_ids(self):
        nclass_trn = self.nclass // self.nfolds
        class_ids_val = [self.fold + self.nfolds * v for v in range(nclass_trn)]
        class_ids_trn = [self.fold + self.nfolds * v for v in range(nclass_trn)]
    ...
    def build_img_metadata_classwise(self):
        with open('./data/%s/coco/%s/fold%d.pkl' % (self.splits_finetune, self.split, self.fold), 'rb') as f:
            img_metadata_classwise = pickle.load(f)
        return img_metadata_classwise
...
class DatasetCOCO_finetune_5shot(Dataset):
    def __init__(self, datapath, fold, transform, split, shot, use_original_imgsize, model_name):
        self.split = 'val' if split in ['val', 'test'] else 'trn'
        self.fold = fold
        self.nfolds = 4
    ...
    def build_img_metadata_classwise(self):
        with open('./data/%s/coco/%s/fold%d_5shot.pkl' % (self.splits_finetune, self.split, self.fold), 'rb') as f:
            img_metadata_classwise = pickle.load(f)
        return img_metadata_classwise
