...
class Visualizer:
    ...
    def initialize(cls, visualize, vispath='./vis/'):
        ...
        cls.colors = {'red': (255, 50, 50), 'blue': (102, 140, 255), 'white': (255, 255, 255)}
    ...
    @classmethod
    def apply_allcolor_mask(cls, image, mask, color):
        r""" Apply mask to the given image. """
        for c in range(3):
    ...
    @classmethod
    def change_mask(cls, mask):
        r""" Apply mask to the given image. """
        new_mask = np.where(mask == 1, 255., 0.)
        return new_mask
    
    @classmethod
    def change_spt_mask(cls, image, mask):
        r""" Apply mask to the given image. """
        for c in range(3):
            image[:, :, c] = np.where(mask == 1, image[:, :, c], 255.)
        return image
