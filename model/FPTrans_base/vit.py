...
import model.FPTrans_base.vit_utils as utils
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='', shot=1, original=False,dataset = 'pascal'):
        """
        Args:
    ...
    def forward(self, x):
        ...
        # fg_token: [B, 1, C]
        # bg_token: [B, k, C]
        # support_mask [B, S, H, W] -> [B, H, W]
        # query_feature:[B, C, H, W] ->[B , h * w, dim]
        # support_features:[B*S, C, H, W] ->[B , h * w, dim],    support_features one-shot
        # support_feature
        x, (fg_token, bg_token) = x
        bank_size, G, embed_dim = self.prompt_tokens.shape
        B = x.shape[0] // (1 + self.shot)
        divider = 1 + self.bg_num * self.shot
        ...
        distill_loss_query_list = []
        distill_loss_sup_list = []
        aux_pred_list = []
        # index = 0
        # list_ad = [1,3,5,7]
        # list_ad = []
        for i, block in enumerate(self.blocks):#2,6,8,10
            # print('i_num', i)
            x = block(x)#
            ...
            #     # import ipdb
            #     # ipdb.set_trace()
            #     # class_token, image_token, fg_token, bg_token = x.split([self.num_tokens,N-self.num_tokens, n1, n2], dim=1)
            #     for_adapter = x.clone()
            #     image_token = for_adapter[:,self.num_tokens:N, :]
            #     #support_mask,support_feature,query_feature
            #     support_feature = image_token[:B*S,:,:]#[B*S, H, W]
            #     query_feature = image_token[B*S:,:,:]#[B*1, H, W]
                
            #     feats_q, feats_sup, distill_loss_q, distill_loss_sup, aux_pred = \
            #         adapter_block[index](query_feature, support_feature, support_mask)
            #     aux_pred_list.append(aux_pred)
            #     distill_loss_query_list.append(distill_loss_q)
            #     distill_loss_sup_list.append(distill_loss_sup)
            #     support_feature_adapter = support_feature + adapter_weight * feats_sup
            #     query_feature_adapter = query_feature + adapter_weight * feats_q
            #     image_token_adapter = torch.cat([support_feature_adapter,query_feature_adapter],dim=0)
            #     # # x = torch.cat([class_token, image_token, fg_token, bg_token],dim=1)
            #     x[:, self.num_tokens:N, :] = image_token_adapter
            #     index += 1
        ...
        return dict(out=x, tokens=tokens, distill_loss_q=distill_loss_query_list, distill_loss_sup=distill_loss_sup_list, aux_pre=aux_pred_list)
